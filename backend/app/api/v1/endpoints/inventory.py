...
async def create_movement(
    ...
    if movement.type == "in":
        # Create new lot for incoming stock
        new_lot = Lot(
        ...
        wine.quantity += movement.quantity
        ...
        if movement.lot_id:
            ...
            if lot.quantity < movement.quantity:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Insufficient stock in lot. Available: {lot.quantity}, requested: {movement.quantity}"
                )
            ...
            if remaining > 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Insufficient stock. Available: {wine.quantity}, requested: {movement.quantity}"
                )
        
        # Check total wine stock
        if wine.quantity < movement.quantity:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
